...
export async function seedLocalCacheFromFirestore(userId: string): Promise<void> {
  try {
    ...
    if (players.length > 0) {
      localStorage.setItem(StorageKeys.players(userId), JSON.stringify(players));
    }
    ...
    // Update games index
    if (gameIds.length > 0) {
      const existingIndex = localStorage.getItem(StorageKeys.gamesIndex(userId));
      const existingIds: string[] = existingIndex ? JSON.parse(existingIndex) : [];
      const mergedIds = [...new Set([...existingIds, ...gameIds])];
      localStorage.setItem(StorageKeys.gamesIndex(userId), JSON.stringify(mergedIds));
    }
