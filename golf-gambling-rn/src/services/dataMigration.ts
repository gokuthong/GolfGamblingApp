...
import { doc, setDoc, getDoc, getDocs, Timestamp, collection, query, where } from 'firebase/firestore';
...
export async function migrateLocalDataToFirestore(userId: string): Promise<void> {
  try {
    ...
    const migrated = await AsyncStorage.getItem(`${MIGRATION_KEY_PREFIX}${userId}`);
    if (migrated) return;
    ...
    await AsyncStorage.setItem(`${MIGRATION_KEY_PREFIX}${userId}`, 'true');
 ...
 */
export async function seedLocalCacheFromFirestore(userId: string): Promise<void> {
  try {
    ...
    const players: Player[] = playersSnapshot.docs.map((d) => ({
      id: d.id,
      ...d.data(),
    } as Player));

    if (players.length > 0) {
      await AsyncStorage.setItem(StorageKeys.players(userId), JSON.stringify(players));
    }
    ...
    for (const gameDoc of gamesSnapshot.docs) {
      ...
      const scores: Score[] = scoresSnapshot.docs.map((d) => ({
        id: d.id,
        ...d.data(),
      } as Score));
    ...
    // Update games index
    if (gameIds.length > 0) {
      const existingIndex = await AsyncStorage.getItem(StorageKeys.gamesIndex(userId));
      const existingIds: string[] = existingIndex ? JSON.parse(existingIndex) : [];
      const mergedIds = [...new Set([...existingIds, ...gameIds])];
      await AsyncStorage.setItem(StorageKeys.gamesIndex(userId), JSON.stringify(mergedIds));
    }
    ...
    if (userDoc.exists()) {
      ...
      await AsyncStorage.setItem(`@user:${userId}`, JSON.stringify({
        ...
        createdAt: uData.createdAt ? toDate(uData.createdAt).toISOString() : new Date().toISOString(),
