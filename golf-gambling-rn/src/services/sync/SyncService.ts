...
class SyncService {
  ...
  async markDirty(entity: DirtyEntity): Promise<void> {
    ...
    dirtySet[key] = entity;
    await AsyncStorage.setItem(StorageKeys.syncDirtyEntities(), JSON.stringify(dirtySet));
  }
  ...
  async markDelete(entity: PendingDelete): Promise<void> {
    ...
    if (!deletes.find((d) => `${d.entityType}:${d.entityId}` === key)) {
      deletes.push(entity);
      await AsyncStorage.setItem(StorageKeys.syncPendingDeletes(), JSON.stringify(deletes));
    }
  }
  ...
  private async getDirtyEntities(): Promise<Record<string, DirtyEntity>> {
    try {
      const stored = await AsyncStorage.getItem(StorageKeys.syncDirtyEntities());
  ...
  private async getPendingDeletes(): Promise<PendingDelete[]> {
    try {
      const stored = await AsyncStorage.getItem(StorageKeys.syncPendingDeletes());
      return stored ? JSON.parse(stored) : [];
    } catch {
      return [];
    }
  ...
  private async syncDirtyEntities(): Promise<void> {
    ...
    await AsyncStorage.setItem(StorageKeys.syncDirtyEntities(), JSON.stringify(remaining));
  }
  ...
  private async syncPendingDeletes(): Promise<void> {
    ...
    await AsyncStorage.setItem(StorageKeys.syncPendingDeletes(), JSON.stringify(remaining));
  }
