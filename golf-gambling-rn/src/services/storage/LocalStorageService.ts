...
class LocalStorageService {
  ...
  async createGame(
    ...
    await AsyncStorage.setItem(StorageKeys.game(gameId), serializeStoredGame(storedGame));
  ...
  async getGame(gameId: string): Promise<Game | null> {
    try {
      const stored = await AsyncStorage.getItem(StorageKeys.game(gameId));
  ...
  async deleteGame(gameId: string): Promise<void> {
    ...
    // Delete game data
    await AsyncStorage.removeItem(StorageKeys.game(gameId));
  ...
  async createHole(holeData: HoleData): Promise<string> {
    ...
    const stored = await AsyncStorage.getItem(StorageKeys.game(holeData.gameId));
    if (!stored) {
      throw new Error('Game not found');
    }
    ...
    await AsyncStorage.setItem(StorageKeys.game(holeData.gameId), serializeStoredGame(storedGame));
  ...
  async updateHole(holeId: string, updates: Partial<HoleData>): Promise<void> {
    ...
    for (const gameKey of gameKeys) {
      const stored = await AsyncStorage.getItem(gameKey);
      if (!stored) continue;
      ...
      if (holeIndex !== -1) {
        ...
        await AsyncStorage.setItem(gameKey, serializeStoredGame(storedGame));
        return;
      }
    }
  ...
  // Note: These are not true streams since AsyncStorage doesn't support real-time updates
  ...
  streamHolesForGame(gameId: string, callback: (holes: Hole[]) => void): () => void {
    ...
    // Return unsubscribe function (no-op for AsyncStorage)
    return () => {
      // No-op: AsyncStorage doesn't have listeners to unsubscribe from
    };
  }
  ...
  async createPlayer(playerData: {
    ...
    await AsyncStorage.setItem(StorageKeys.players(playerData.userId), JSON.stringify(players));
  ...
  async createPlayerLegacy(
    ...
    await AsyncStorage.setItem(StorageKeys.players(effectiveUserId), JSON.stringify(players));
  ...
  async getAllPlayers(userId?: string): Promise<Player[]> {
    try {
      if (userId) {
        const stored = await AsyncStorage.getItem(StorageKeys.players(userId));
        if (!stored) return [];
        const players: Player[] = JSON.parse(stored);
  ...
  async getAllCourses(userId?: string): Promise<Course[]> {
    try {
      if (userId) {
        const stored = await AsyncStorage.getItem(StorageKeys.courses(userId));
        if (!stored) return [];
  ...
  async getNextUserNumber(): Promise<string> {
    try {
      const stored = await AsyncStorage.getItem('@userCounter');
      const currentNumber = stored ? parseInt(stored) : 0;
      const nextNumber = currentNumber + 1;

      await AsyncStorage.setItem('@userCounter', nextNumber.toString());
  ...
  async getUserProfile(userId: string): Promise<any> {
    try {
      const stored = await AsyncStorage.getItem(`@user:${userId}`);
      if (!stored) return null;
  ...
  private async addToGamesIndex(userId: string, gameId: string): Promise<void> {
    ...
    if (!gameIds.includes(gameId)) {
      gameIds.push(gameId);
      await AsyncStorage.setItem(indexKey, JSON.stringify(gameIds));
    }
  }
  ...
  private async removeFromGamesIndex(userId: string, gameId: string): Promise<void> {
    ...
    await AsyncStorage.setItem(indexKey, JSON.stringify(filteredIds));
  }
  ...
  private async updateGamesIndex(userId: string): Promise<void> {
    ...
    await AsyncStorage.setItem(StorageKeys.gamesIndex(userId), JSON.stringify(sortedGameIds));
  }
  ...
  async createGuestProfile(deviceId: string): Promise<any> {
    ...
    await AsyncStorage.setItem(`@user:${deviceId}`, JSON.stringify({
      ...guestUser,
  ...
  async upgradeGuestToUser(guestId: string, userData: any): Promise<void> {
    ...
    await AsyncStorage.setItem(`@user:${guestId}`, JSON.stringify(userData));
  }
  ...
  async getUserRole(userId: string): Promise<string> {
    try {
      const stored = await AsyncStorage.getItem(StorageKeys.userRole(userId));
  ...
  async setUserRole(userId: string, role: string): Promise<void> {
    await AsyncStorage.setItem(StorageKeys.userRole(userId), role);

    // Also update in user profile
    const profile = await this.getUserProfile(userId);
    if (profile) {
      await AsyncStorage.setItem(`@user:${userId}`, JSON.stringify({
        ...profile,
        role,
  ...
  async getApprovalStatus(userId: string): Promise<string> {
    try {
      const stored = await AsyncStorage.getItem(StorageKeys.userApprovalData(userId));
      if (stored) {
  ...
  async setApprovalStatus(userId: string, status: string): Promise<void> {
    ...
    await AsyncStorage.setItem(StorageKeys.userApprovalData(userId), JSON.stringify(approvalData));
  ...
  async addPendingUser(userId: string, userData: any): Promise<void> {
    ...
    await AsyncStorage.setItem(StorageKeys.pendingUsers(), JSON.stringify(pendingUsers));
  }

  async getPendingUsers(): Promise<any[]> {
    try {
      const stored = await AsyncStorage.getItem(StorageKeys.pendingUsers());
      if (!stored) return [];
  ...
  async removePendingUser(userId: string): Promise<void> {
    ...
    await AsyncStorage.setItem(StorageKeys.pendingUsers(), JSON.stringify(
      filtered.map(u => ({
        ...u,
